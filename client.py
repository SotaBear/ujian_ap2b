...
def request_book_status_from_api(isbn, current_thread_name):
    ...
    log_client_activity_safe(current_thread_name, f"Mengirim permintaan GET untuk isbn: {isbn} ke {target_url}")

    try:
        response = requests.get(target_url, timeout=5)
        if response.status_code == 200:
            data = response.json()
            log_client_activity_safe(current_thread_name, f"Berhasil! Buku '{data.get('isbn', isbn)}: Judul {data.get('title', 'N/A')}, Status {data.get('status', 'N/A')}, Lokasi {data.get('location', 'N/A')}")
        ...
        log_client_activity_safe(current_thread_name, f"Timeout! Permintaan untuk ISBN {isbn} melebihi batas waktu.")
    except requests.exceptions.RequestException as e:
        log_client_activity_safe(current_thread_name, f"Error permintaan umum untuk ISBN {isbn}: {e}")
    except Exception as e:
        log_client_activity_safe(current_thread_name, f"Error tak terduga saat memproses ISBN {isbn}: {e}")
    finally:
        log_client_activity_safe(current_thread_name, f"Selesai pemrosesan permintaan untuk ISBN: {isbn}")
    # =================================================
